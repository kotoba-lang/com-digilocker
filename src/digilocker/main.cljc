(ns digilocker.main
  "Kotodama WASM entrypoint for the Digilocker clean-room actor (L4) — Clojure port.
  ...
  facts (`digilocker.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "digilocker")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"digilocker.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "digilocker-compat" :tier tier :entities entities} 200])
...
(defn start! [] :digilocker-compat/ready)
